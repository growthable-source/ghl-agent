...
> Voxility installs AI agents directly into your sub-account. They reply to inbound messages across SMS, Messenger, Instagram, email, voice, and web chat — and update contacts, move opportunities, book appointments, and trigger workflows the same way your team would. Set up in under five minutes. No data migration, no second inbox to check.

## Full description

> ### AI agents that actually use your CRM
>
> Voxility is an AI agent platform built for LeadConnector. Every agent you create reads and writes your existing data — contacts, conversations, pipelines, calendars, tags, custom fields, workflows — through the tools your team already uses. Nothing to migrate. No parallel inbox.
>
...
>
> Install from the marketplace. We provision a Voxility workspace tied to your sub-account automatically, then walk you through:
>
...
> ### Pricing
>
> Free 7-day trial, then plans from US$X/month. Full pricing at voxility.ai/pricing.
>
> ### Support
>
> Docs: docs.voxility.ai
> Email: support@voxility.ai
...
Voxility requests these scopes when you install:
...
1. **Install** — One click from the marketplace. Voxility provisions a workspace tied to your sub-account.
...
You can disconnect Voxility from your sub-account at any time from Settings → Integrations. Your contacts, conversations, and pipeline data stay where they are — we never delete anything from your CRM. Voxility just stops listening.
...
- **Support email**: confirm `support@voxility.ai` is monitored (or use a different inbox).
- **Docs URL**: `docs.voxility.ai` is hypothetical — replace with the actual docs domain when it's live, or drop the line if there are no public docs yet.
...
- **App icon**: square Voxility wordmark at 512×512 (the marketplace usually requires PNG, no transparency in some categories).
