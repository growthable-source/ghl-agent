...
### Task 4: Portal embed cookie — DROPPED (already shipped upstream)

While this plan was in flight, `main` gained the equivalent feature:
`lib/portal-auth.ts` sets a `voxility_portal_embed` SameSite=None twin
cookie on every production portal login and `getPortalSession()` reads
both cookie names, so framed portal logins already work with no
middleware changes. This task was skipped at execution time.
