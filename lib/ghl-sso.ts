/**
 * Decryption for GHL Custom App SSO payloads.
 *
 * When Voxility is embedded as an iframe inside GHL (via a Custom Menu
 * Link in the marketplace listing), GHL's parent frame posts an
 * encrypted blob containing the active user's identity. We decrypt it
 * with the Shared Secret (a.k.a. "SSO Key") from the marketplace app
 * settings, trust the resulting user info because GHL signed it with our
 * secret, and mint a Voxility session from it.
 *
 ...
 * of key + 16 bytes of IV for AES-256-CBC. This is what GHL emits today;
 * if they switch to authenticated encryption in the future, swap the
 * algorithm below — the call sites won't change.
 */
...
export interface DecryptedSsoPayload {
  ...
  userName?: string
  email?: string
  // Anything else GHL stuffs in there. Treated as untrusted until
  // we map it to our own ids.
  [key: string]: unknown
}
...
export function decryptSsoBlob(encryptedBase64: string, sharedSecret: string): DecryptedSsoPayload {
  if (!sharedSecret) {
    throw new Error('GHL_SSO_KEY is not configured. Set it in env to the Shared Secret from your marketplace app settings.')
  }
  const raw = Buffer.from(encryptedBase64, 'base64')
  if (raw.length < 16 || raw.subarray(0, 8).toString('utf8') !== 'Salted__') {
    throw new Error('Encrypted payload is not in OpenSSL "Salted__" format — GHL may have changed their SSO encryption scheme.')
  }
