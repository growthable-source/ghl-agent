...
export async function runBuild(args: RunBuildArgs): Promise<void> {
  ...
  for (let iter = 1; iter <= build.maxIterations; iter++) {
    ...
    try {
      ...
      // Style is now picked by the spec generator itself, looking at
      // the actual generated assets (hero photo, illustrations) and
      // deciding background + primary_color from what's IN those
      // images. No regex on "design_vibe" strings here — the designer
      // looks at the mood board and picks coherent page chrome.
