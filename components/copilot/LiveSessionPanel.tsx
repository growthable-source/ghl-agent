...
export default function LiveSessionPanel({
  ...
  // Visual pointer (annotate_screen). Coordinates are PERCENTAGES of the
  // shared frame as the model last saw it.
  ...
  // Aspect ratio of the actual shared video. The in-panel preview is
  // sized to THIS (not a fixed 16:9), so object-contain never letterboxes
  // and a percentage-positioned marker lands where the model meant it —
  // the letterbox drift was exactly why screen-marking was pulled before.
  ...
  // The annotation marker lives on the preview INSIDE this panel — but
  // during a session the user is working in their OWN product in another
  // tab, so a marker here is invisible to them. The pop-out composites
  // preview + marker onto a canvas and floats it always-on-top via the
  // video PiP API, so "look at the marker" works wherever they are. The
  // canvas is sized to the video's own dimensions, so the marker math
  // here has no letterbox to fight.
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      provider.onToolCall = async call => {
        ...
        // the preview (and the floating pop-out) and answer the model
        // immediately. NEVER report success the model can't see — the
        // old fake "Annotation displayed." stub is what made it insist
        // "I'm marking it" when nothing appeared.
        if (call.name === 'annotate_screen') {
          ...
          const popOutOpen = typeof document !== 'undefined' && !!document.pictureInPictureElement
          return {
            result: popOutOpen
              ? 'Marker shown on the floating preview. Tell the user to glance at the pop-out window.'
              : 'Marker shown on the live-help preview. Tell the user to glance at this tab — or to open the floating preview so it stays on top while they work.',
          }
        }
      ...
      {phase.kind === 'live' && (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <div className="lg:col-span-2 space-y-4">
            {/* Sized to the SHARED video's own aspect ratio (not a fixed
                16:9) so object-contain never letterboxes — a marker
                positioned by percentage then lands exactly where the
                model meant it. True black background for the brief moment
                before the stream's dimensions are known. */}
