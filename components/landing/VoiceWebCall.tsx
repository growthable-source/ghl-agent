 ...
 * behind a quick lead-capture form. The trigger button opens a MODAL (dark
 * backdrop, solid card) rather than expanding inline — so it never shifts
 * the page, and there's no backdrop-blur recompositing over the live
 * animation (which made typing lag). We save the lead first (so a bailed
...
export default function VoiceWebCall({
  ...
  const close = useCallback(() => {
    if (state === 'live' || state === 'connecting') void endCall('ended')
    reset()
    setOpen(false)
  }, [state, endCall, reset])
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center p-4 vox-backdrop-fade"
          style={{ background: 'rgba(0,0,0,0.66)' }}
          onClick={close}
        >
          <div
            className="relative w-full max-w-sm rounded-2xl p-6"
            style={{ background: 'linear-gradient(160deg, #17131c 0%, #0e0c14 100%)', border: '1px solid rgba(255,255,255,0.14)', boxShadow: '0 30px 80px -20px rgba(0,0,0,0.8)', maxHeight: '92vh', overflowY: 'auto' }}
  ...
  return (
    <div className="mb-3">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} required placeholder={placeholder} autoComplete={autoComplete}
        ...
        style={{ background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(255,255,255,0.18)', color: '#fff' }} />
    </div>
  )
}
