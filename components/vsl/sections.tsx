  ...
  return (
    <section className="relative overflow-hidden px-4 pt-20 pb-24 md:pt-28 md:pb-32">
      ...
      <div className={`mx-auto ${usingImage ? 'max-w-6xl' : 'max-w-4xl'} grid items-center gap-12 ${usingImage ? 'md:grid-cols-2' : ''}`}>
        <div className={usingImage ? 'text-left' : 'text-center'}>
          ...
          {s.subheadline && (
            <p
              ...
              style={{ fontSize: 'clamp(1.05rem, 1.1vw + 0.6rem, 1.35rem)', lineHeight: 1.5 }}
            >
              {s.subheadline}
            </p>
          )}
          ...
          {s.cta_label && (
            <div className={`mt-8 ${usingImage ? '' : 'flex justify-center'}`}>
              <a
                href="#form"
          ...
          {s.trust_badges && s.trust_badges.length > 0 && (
            <div className={`mt-8 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm ${usingImage ? '' : 'justify-center'}`}
              style={{ color: 'rgba(0,0,0,0.55)' }}>
              {s.trust_badges.map((b) => (
                <span key={b} className="flex items-center gap-1.5">
                  <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24" style={{ color: 'var(--brand, #0A84FF)' }}>
        ...
        {/* Right-column media. Either a YouTube/Vimeo embed (centered
            full-width below the copy when usingImage=false), an operator-
            uploaded hero image, or the AI-generated hero photo. */}
        {usingImage && (
          <div className="relative">
            ...
            <div className="overflow-hidden rounded-3xl border border-black/5 shadow-2xl">
              {/* Photographic image. AI-generated hero takes precedence
                  unless the operator explicitly attached a media URL. */}
              <img
                src={heroImage?.url ?? aiHero}
                alt={heroImage?.alt ?? ''}
...
export function SectionRenderer({
  ...
  switch (section.type) {
    case 'hero':
      return <HeroBlock s={section} images={images} />
    case 'problem':
