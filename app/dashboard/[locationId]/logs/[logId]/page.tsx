...
export default async function LogDetailPage({
  ...
  params: Promise<{ locationId: string; logId: string }>
}) {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            {log.agent && (
              <Link href={`/dashboard/${locationId}/agents/${log.agent.id}`} className="text-xs text-zinc-500 hover:text-white transition-colors">
                {log.agent.name}
              </Link>
            )}
          </div>
