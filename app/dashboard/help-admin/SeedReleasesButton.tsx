...
/**
 * One-click trigger for /api/help/seed-releases. Creates the "What's new"
 * category and every article under it, or updates them in place if they
 * already exist. Idempotent.
 */
export default function SeedReleasesButton() {
  ...
  async function run() {
    ...
    setStatus(null)
    try {
      const res = await fetch('/api/help/seed-releases', { method: 'POST' })
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        ...
        title="Seed or reseed the What's New category from lib/help-seed-releases.ts"
      >
        {busy ? 'Seeding…' : "Seed What's New docs"}
      </button>
    </div>
  )
}
